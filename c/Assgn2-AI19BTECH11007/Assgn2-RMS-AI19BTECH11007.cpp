...
int Min(int deadline[],int remaing[])
{
    ...
    for(int i=0;i<process_num;i++)
    {
        ...
        {
            if(processing_time[i]<min_time)
            {
                min_time=processing_time[i];
                min_num=i;
            }
        }  
    }
...
void RMS(int max)
{
    ofstream rms_file ( "RMS-log.txt");
    ofstream rms_stats("RMS-stats.txt");
    ...
    for(int i=0; i<max;i++)
    {
        ...
        if(prev!=next)
        {
            ...
            {
                rms_file<<"Process"<<pid[prev]<<"completed excuting at time"<<i<<"\n";
            }
            if(pid[prev]!=0 && pid[next]!=0 && remain[prev]!=0)
            {
                rms_file<<"Process"<<pid[prev]<<"is preemepted by process"<<pid[next]<<"at time"<<i<<"\n";
            }
            if(next!=-1)
            {
                rms_file<<"Process"<<pid[next]<<"Started excuting at time"<<i<<"\n";
            }
            if(next==-1)
            {
                rms_file<<"cpu is idle from"<<i<<"\n";
            }
            prev=next;
        }
    }
    ...
    rms_stats<<"num of prcoess came into the system :"<<sum2<<"\n";
    rms_stats<<"num of process that sucessfully completed :"<<sum2-count<<"\n";
    rms_stats<<"num of process that missed deadline :" <<count<<"\n";
    rms_stats<<"The avrage waiting time :"<<average<<"\n";
    rms_file.close();
    rms_stats.close();
}
